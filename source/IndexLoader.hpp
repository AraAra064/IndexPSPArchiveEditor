...
namespace IndexLoader
{
	...
	std::string GetFileExtStr(std::vector<uint8_t>& fileData)
	{
		std::string header = std::string((char*)&fileData[0]);
		if (header.size() > 4){header.resize(4);
		}
		for (uint8_t c : header)
		{
			if (!isalnum(c))
			{
				header = "bin";
	...
	std::vector<std::string> LoadList(std::vector<uint8_t> &archiveData, std::deque<std::vector<uint8_t>>& newFiles, std::vector<AssetArchive::FileInfo>& fileInfo, bool isEncoded, std::string fileName)
	{
		...
		if (readFile.is_open())
		{
			fList.resize(AssetArchive::GetNumberOfFiles(archiveData));
			std::string line, dir = ".";
			...
			while (std::getline(readFile, line))
			{
				switch (line[0])
				{
					...
					case '=': //Size
						newFiles.resize(strtoul(line.c_str() + 1, nullptr, 0));
						fList.resize(newFiles.size());
						continue;
				...
				if (rd.size() < 2)
				{
					std::cout << "Skipping \"" << line << "\". Incorrect format." << std::endl;
					continue;
				}

				uint32_t fileIndex = strtoul(rd[0].c_str(), nullptr, 0) % newFiles.size();
				std::string fileName = dir + '\\' + rd[1]; //dir+'\\'+
				fList[fileIndex] = rd[1];

				bool b = (fileIndex < AssetArchive::GetNumberOfFiles(archiveData));
				std::vector<uint8_t> oldData = b ? AssetArchive::GetFileData(archiveData, fileInfo, fileIndex, isEncoded) : std::vector<uint8_t>();
				int oW = 0;
				int oH = 0;
				...
				newFiles[fileIndex] = UpdateFile(fileName, oW, oH); //".NewFiles\\"+
			}
		}
